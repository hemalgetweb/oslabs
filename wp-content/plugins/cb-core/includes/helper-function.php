...
function oslabs_drop_posts($post_type)
{
    $args = array(
        'numberposts' => -1,
        'post_type'   => $post_type
...
function cb_core_elementor_review_star_rating_3($star_count) {
    $html = '';
    $html .= '<div class="oslabs-fz-rating-wrap-4">';
...
function cb_core_elementor_review_star_rating_4($star_count) {
    $html = '';
    $html .= '<div class="oslabs-fz-testimonial-author-rating-6">';
    for($i=1;$i<=$star_count; $i++) {
        $html .= '<i class="fa-solid fa-star"></i>';
    }
    for($i = 5;$i > $star_count; $i--) {
...
function oslabs_drop_cat_featured($tax)
{
    $args = [
        'taxonomy' => $tax,
        'hide_empty' => true,
...
function cb_core_wc_get_review_2($anchor = false, $class = 'oslabs-woo-product-rating pb-10')
{
    global $product;
    $count = $product->get_average_rating();
    $html = '';
...
function cb_core_wc_get_review_layout($anchor = false, $class = 'oslabs-fz-rating-2 mb-4-px')
{
    global $product;
    $count = $product->get_average_rating();
    $html = '';
...
function cb_core_wc_get_trending_banner_review($anchor = false, $class = 'oslabs-fz-trending-product-rating')
{
    global $product;
    $count = $product->get_average_rating();
    $html = '';
...
{ ?>
    <div class="cart-option cart-option-bottom">
        <ul>
            <li>
                <div class="oslabs-action-cart-btn">
                    <?php
            ...
            <li>
                <div class="oslabs-action-wishlist-btn">
                    <?php
            ...
            <li>
                <div class="oslabs-action-compare-btn">
                    <?php
...
?>
    <div class="oslabs-fz-product-action-list-2 has-pos">
        <?php if (function_exists('woosw_plugin_activate')) : ?>
            <?php echo do_shortcode('[woosw class="oslabs-fz-wishlist-btn-2342" id="' . $_product_id . '"]'); ?>
        <?php endif; ?>
...
{ ?>
    <div class="oslabs-fz-product-action-wrapper-2 has-pos fz-action-layout">
        <?php
        if (function_exists('woosw_plugin_activate')) {
            echo do_shortcode('[woosw id="' . get_the_ID() . '"]');
        }
...
{ ?>
    <div class="oslabs-fz-product-box-action-6 has-pos">
        <?php
        if (function_exists('woosw_plugin_activate')) {
            echo do_shortcode('[woosw id="' . get_the_ID() . '"]');
        }
...
{ ?>
    <div class="oslabs-fz-product-action-wrapper-5 has-pos">
        <?php woocommerce_template_loop_add_to_cart_layout(); ?>
        <?php
        if (function_exists('woosq_init')) {
            echo do_shortcode('[woosq id="' . get_the_ID() . '"]');
...
{ ?>
    <div class="oslabs-fz-product-box-4-actions">
...
{
    $html = '';
    $html .= '<div class="oslabs-fz-rating-2 mb-4-px wow fadeInUp">';
    for ($i = 1; $i <= $star_count; $i++) {
        $html .= '<i class="fa-light fa-star active"></i>';
    }
    for ($i = 5; $i > $star_count; $i--) {
...
{
    $html = '';
    $html .= '<div class="oslabs-fz-product-rating-6">';
    for ($i = 1; $i <= $star_count; $i++) {
        $html .= '<i class="fa-solid fa-star"></i>';
    }
    for ($i = 5; $i > $star_count; $i--) {
...
{
    $html = '';
    $html .= '<div class="rating"><div class="oslabs-woo-product-rating">';
    for ($i = 1; $i <= $star_count; $i++) {
        $html .= '<i class="fa-solid fa-star active"></i>';
    }
    for ($i = 5; $i > $star_count; $i--) {
