...
if ( ! class_exists( 'Oslabs_Walker_Comment' ) ) {
	class Oslabs_Walker_Comment extends Walker_Comment {
				...
				<div id="div-comment-<?php comment_ID(); ?>" class="comment-list-item d-flex">
                    ...
                    echo "<div class='comment-list-content'>";
                        echo "<div class='comment-contenet'>";
                            echo '<div class="oslabs-fz-comment-top-head-778">';
                                ...
                                echo '<div class="comment-btn-wrapper">';
                                    ...
                                    $comment_timestamp = sprintf( __( '%1$s at %2$s', 'oslabs' ), get_comment_date( '', $comment ), get_comment_time() );
                                    printf(
                                        '<a href="%s"><time class="oslabs-comment-time" datetime="%s" title="%s">%s</time></a>',
                            ...
                            echo '<p class="comment-text">';
                                ...
                                if ( '0' === $comment->comment_approved ) {
                                    ?>
                                    <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'oslabs' ); ?></p>
                                    <?php
                                }
                            echo '</p>';
                            $comment_reply_link = get_comment_reply_link(
                                array_merge(
                                    ...
                                    array(
                                        ...
                                        'before'    => '<div class="oslabs-fz-comment-replay-btn-448">',
                                        'after'     => '</div>',
                                    )
                                )
                            );
                            $by_post_author = oslabs_is_comment_by_post_author( $comment );
